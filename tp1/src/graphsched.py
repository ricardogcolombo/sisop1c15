	...
	for l in fin:
		...
		if len(vls) == 4 and vls[0] == 'CPU':
			...
			if vls[2] != "-1" or int(vls[3]) == idleCore:
				result.append((CMDS.index(vls[0]), int(vls[1]), vls[2], int(vls[3])))
			core = int(vls[3])
			if (cores <=core):
				cores = core+1
		else:
			result.append((CMDS.index(vls[0]), int(vls[1]), vls[2], -1))

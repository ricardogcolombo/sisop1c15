...
static vector<ptsk> lote_global;
...
char type(int pid) { return lote_global[pid].type; }
unsigned int period(int pid) { return lote_global[pid].period; }
unsigned int declared_cputime(int pid) { return lote_global[pid].cputime; }
unsigned int total_tasks() { return lote_global.size(); }
...
void simulate(SchedBase& sch, std::vector<ptsk>& lote, const Settings& settings) {
	...
	while (finished < n || context_remain) {
		...
		while (!load.empty() && load.top().first >= -(int)cur_time) {
			...
			tsks[pid].running = 1;
			sch.load(pid);
			cout << "LOAD " << cur_time << " " << tsks[pid].type << pid << endl;
		}
				...
				if (cpu_pid == IDLE_TASK) {
                    ...
                    switch (tsks[cpu_pid].blk) {
                        case ST_EXIT:
                            ...
                            cout << "EXIT " << cur_time << " " << tsks[cpu_pid].type << cpu_pid << " " << cpu << endl;
                        ...
                        case ST_IO:
                            ...
                            cout << "BLOCK " << cur_time << " " << tsks[cpu_pid].type << cpu_pid << endl;
		...
		for(int j=0; j<(int)to_unblock.size(); j++) cout << "UNBLOCK " << cur_time << " " << tsks[to_unblock[j]].type << to_unblock[j] << endl;
		context_remain= 0;
		...
		for(int i= 0; i < contexts.size(); i++){
			...
			if (contexts[i].remaining /*context_remain*/) {
				...
				if(contexts[i].pid != IDLE_TASK)
					cout << "CPU "<< cur_time << " " << tsks[contexts[i].pid].type << contexts[i].pid << " " << i <<endl;
				else
